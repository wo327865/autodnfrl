...
def main() -> None:
    ...
    workflow = AutoDNF(client, debug=args.debug)
    if args.command == "scan":
        ...
        if args.battle:
            workflow.run_all_characters()
        else:
            workflow.run_to_party(battle=False)
    elif args.command == "realm":
        workflow.continue_from_realm_selection(battle=args.battle)
    elif args.command == "party":
        workflow.configure_party()
        if args.battle:
            workflow.run_battle(start_by_entering=True)
    else:
        workflow.run_battle(start_by_entering=False)
